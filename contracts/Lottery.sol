...
contract Lottery {
  ...
  event BET(uint256 index, address indexed bettor, uint256 amount, bytes challenges, uint256 answerBlockNumber);
  ...
  //Bet
  /**
   * @dev 베팅 함수. 유저는 0.005이더 전송. 베팅용 글자 1byte 전송.
   * 큐에 저장된 베팅 정보는 이후 distribute 함수에서 해결된다.
  ...
  */
  function isMatch(bytes memory challenges, bytes32 answer) public pure returns (BettingResult) {
    ...
    bytes32 c1 = bytes32(challenges);
    bytes32 c2 = bytes32(challenges);
